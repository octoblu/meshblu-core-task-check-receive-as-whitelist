...
describe 'CheckAsWhitelist', ->
  ...
  describe '->do', ->
    describe 'when called with a valid job', ->
      beforeEach (done) ->
        @whitelistManager.canReceiveAs.yields null, true
        job =
          metadata:
            auth:
              uuid: 'green-blue'
    ...
    describe 'when called and the canReceiveAs yields an error', ->
      beforeEach (done) ->
        @whitelistManager.canReceiveAs.yields new Error "black-n-black"
        job =
          metadata:
            auth:
              uuid: 'puke-green'
